...
class BlazePose(BlazeDetector):
    ...
    def forward(self, x):
        ...
        raw_box_tensor[..., 4] = raw_box_tensor[..., 4] / self.x_scale * self.anchors[:, 2] + self.anchors[:, 0]
        raw_box_tensor[..., 5] = raw_box_tensor[..., 5] / self.y_scale * self.anchors[:, 3] + self.anchors[:, 1]

        raw_box_tensor[..., 10] = raw_box_tensor[..., 10] / self.x_scale * self.anchors[:, 2] + self.anchors[:, 0]
        raw_box_tensor[..., 11] = raw_box_tensor[..., 11] / self.y_scale * self.anchors[:, 3] + self.anchors[:, 1]
